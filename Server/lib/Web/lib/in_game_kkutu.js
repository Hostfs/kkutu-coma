...
var audioContext;
try {
	audioContext = (window.AudioContext || window.webkitAudioContext) ? new (window.AudioContext || window.webkitAudioContext)() : false;
} catch (e) {
	audioContext = false;
}
...
function addonNickname($R, o){
	...
	if(o.equip['BDG'] == "b1_gm" || o.admin) $R.addClass("x-gm");
}
...
function processNormal(word, mean, theme){
	var $R = $("<label>").addClass("word");
	if(theme){
		var themes = theme.split(',').map(function(_t){
			return L['theme_' + _t];
		});
		themes.forEach(function(_t){
			if(_t) $R.append($("<label>").addClass("word-theme").html(_t));
		});
	}
	$R.append($("<label>").addClass("word-m3-body").html(mean));
	return $R;
}
function processWord(word, _mean, _theme, _wcs){
	if(!_mean || _mean.indexOf("＂") == -1) return processNormal(word, _mean, _theme);
	var $R = $("<label>").addClass("word");
