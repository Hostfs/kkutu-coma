...
function processAdmin(id, value){
	...
	switch(cmd){
		...
		case "kill":
			...
			return null;
		case "nick":
			var targetClient = DIC[id];
			if(targetClient){
				var newName = value.trim();
				if(newName.length > 0 && newName.length <= 15){
					MainDB.users.update([ '_id', targetClient.id ]).set([ 'title', newName ]).on(function(){
						var oldName = targetClient.profile.title || targetClient.profile.name;
						delete DNAME[oldName.replace(/\s/g, "")];
						
						targetClient.profile.title = newName;
						targetClient.profile.name = newName;
						DNAME[newName.replace(/\s/g, "")] = targetClient.id;
						
						MainDB.session.findOne([ '_id', targetClient.key ]).on(function($sessionBody){
							if ($sessionBody && $sessionBody.profile) {
								$sessionBody.profile.title = newName;
								$sessionBody.profile.name = newName;
								MainDB.session.update([ '_id', targetClient.key ]).set([ 'profile', $sessionBody.profile ]).on();
							}
						});
						
						targetClient.send('yell', { value: "닉네임이 '" + newName + "'(으)로 즉시 변경되었습니다!" });
						KKuTu.publish('conn', { user: targetClient.getData() });
					});
				}else{
					targetClient.send('yell', { value: "올바르지 않은 닉네임 길이입니다. (1~15자)" });
				}
			}
			return null;
		case "forcenick":
			...
			if(parts.length >= 2){
				var oldName = parts[0];
				var newName = parts[1];
				var targetId = DNAME[oldName.replace(/\s/g, "")];
				var targetClient = DIC[targetId];
				if(targetClient){
					if(newName.length > 0 && newName.length <= 15){
						MainDB.users.update([ '_id', targetClient.id ]).set([ 'title', newName ]).on(function(){
							delete DNAME[oldName.replace(/\s/g, "")];
							...
							targetClient.send('yell', { value: "관리자에 의해 닉네임이 '" + newName + "'(으)로 강제 변경되었습니다!" });
							if(id !== targetClient.id && DIC[id]){
								DIC[id].send('yell', { value: "'" + oldName + "' 님의 닉네임을 '" + newName + "'(으)로 강제 변경했습니다." });
							}
							KKuTu.publish('conn', { user: targetClient.getData() });
						});
					}else{
						if(DIC[id]) DIC[id].send('yell', { value: "올바르지 않은 닉네임 길이입니다. (1~15자)" });
					}
				}else{
					if(DIC[id]) DIC[id].send('yell', { value: "해당 닉네임('" + oldName + "')을 가진 온라인 유저를 찾을 수 없습니다." });
				}
			}else{
				if(DIC[id]) DIC[id].send('yell', { value: "사용법: #forcenick [이전닉네임] [바꿀닉네임]" });
			}
			return null;
		case "tailroom":
			if(temp = ROOM[value]){
...
function processClientRequest($c, msg) {
	...
	switch (msg.type) {
		...
		case 'talk':
			...
			if (msg.value.startsWith('/닉네임 ') || msg.value.startsWith('/nick ')) {
				if ($c.guest) {
					$c.send('yell', { value: "게스트 계정은 닉네임을 변경할 수 없습니다." });
					break;
				}
				var newName = msg.value.slice(msg.value.indexOf(' ') + 1).trim();
				var nickRegex = /^[a-zA-Z0-9가-힣ㄱ-ㅎㅏ-ㅣ]{2,10}$/;
				if (!nickRegex.test(newName)) {
					$c.send('yell', { value: "닉네임은 한글, 영어, 숫자만 사용 가능하며 2~10자여야 합니다." });
					break;
				}
				
				MainDB.users.findOne([ 'title', newName ]).on(function($existingUser){
					if ($existingUser) {
						$c.send('yell', { value: "이미 존재하는 닉네임입니다." });
					} else {
						MainDB.users.update([ '_id', $c.id ]).set([ 'title', newName ]).on(function(){
							var oldName = $c.profile.title || $c.profile.name;
							delete DNAME[oldName.replace(/\s/g, "")];
							
							$c.profile.title = newName;
							$c.profile.name = newName;
							DNAME[newName.replace(/\s/g, "")] = $c.id;
							
							MainDB.session.findOne([ '_id', $c.key ]).on(function($sessionBody){
								if ($sessionBody && $sessionBody.profile) {
									$sessionBody.profile.title = newName;
									$sessionBody.profile.name = newName;
									MainDB.session.update([ '_id', $c.key ]).set([ 'profile', $sessionBody.profile ]).on();
								}
							});
							
							$c.send('yell', { value: "닉네임이 '" + newName + "'(으)로 즉시 변경되었습니다!" });
							KKuTu.publish('conn', { user: $c.getData() });
						});
					}
				});
