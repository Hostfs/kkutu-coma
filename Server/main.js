...
const PugInit = require('electron-pug');
...
Runner.send = (...argv) => {
	try {
		if (mainWindow && !mainWindow.isDestroyed() && mainWindow.webContents) {
			mainWindow.webContents.send(...argv);
		}
	} catch (err) {
		console.error("Error in Runner.send:", err);
	}
};

function main(){
	// Initialize Pug here after the Electron app is fully ready
	PugInit({ pretty: true }, {
		version: PKG.version,
